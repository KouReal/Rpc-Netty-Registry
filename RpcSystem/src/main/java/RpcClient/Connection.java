...
public class Connection {
	...
	public void setIpaddr(String ip, int targetPort) {
		this.targetIP = ip;
	...
	public Connection(String targetIP, int targetPort, Bootstrap bootstrap, NormalConfig normalConfig) {
		setIpaddr(targetIP, targetPort);
		this.uniformconfig = normalConfig.getUniformconfig();
		DEFAULT_RECONNECT_TRY = uniformconfig.getClientReconnectTry();
		DEFAULT_CONNECT_TIMEOUT = uniformconfig.getClientConnectTimeout();
	}
	...
	public Channel getChannel() throws RpcServiceDisconnectException {
		...
		if (ch != null) {
			...
			try {
				...
				LOGGER.info("当前线程:{}进入wait等待channel队列,最多等待{}毫秒", Thread.currentThread().toString(),
	...
	public Channel Channel() {
		...
		try {
			LOGGER.info("thread:{} in Connection.getChannel trying readlock", Thread.currentThread().toString());
	...
	public void unbind() {
		try {
			LOGGER.info("thread:{} unbind channel:{} Connection", Thread.currentThread(), this.channel);
